...
public class PillImgHandler {
	...
	public List<PillImgVO> uploadFiles(MultipartFile[] files) {
		...
		for(MultipartFile file : files) {
			PillImgVO pivo = new PillImgVO();
			pivo.setPillImgSaveDir(today);
			pivo.setPillImgSize(file.getSize());
			
			String originalFileName = file.getOriginalFilename();
			String fileName = originalFileName.substring(originalFileName.lastIndexOf(File.separator)+1);
			
			log.info(">>> fileName >>> {}", fileName);
			pivo.setPillImgName(fileName);
			
			UUID uuid = UUID.randomUUID();
			String uuidstr = uuid.toString();
			pivo.setPillImgId(uuidstr);
			...
			// D:\User\_myProject\_java\_fileUpload\\2024\\02\\21\\1111_apple.jpg
			...
			try {
				...
				if(isImageFile(storeFile)) {
					pivo.setPillImgType(1);	// 이미지 파일은 타입이 1
					// 썸네일 생성
